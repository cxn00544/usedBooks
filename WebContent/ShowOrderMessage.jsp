...
window.addEventListener('load',function(){
	 var overOrderbtn=document.querySelector('.overOrderbtn');
     ...
     <tbody>
        <c:forEach var="ordermessage" items="${OrderMessageList}">
            <tr>
              ...
              <c:set var="status" scope="session" value="${ordermessage.status}"/>
	          <c:if test="${status == 0}">
	          	<td scope="row" style="color:#ff5000;">等待发货</td>
	          </c:if>
               ...
               <c:if test="${status == 3}">
				...
				<c:if test="${status == 0}">
				  <td> 
	              	 <a href="Confirmorder?bookId=${ordermessage.bookId}&&userId=${user.userId}&&orderNum=${ordermessage.orderNum}" type="button" class="cancelbutton"  onclick="rejectbutton">确定收货</a>
	              </td>
	               <td>
	               	 <a href="ConfirmCeceipt?bookId=${ordermessage.bookId}&&userId=${user.userId}&&orderNum=${ordermessage.orderNum}" type="button" class="cancelbutton"  onclick="cancelbutton">退货</a>
	              </td>
				</c:if>
				<c:if test="${status == 2}">
				<td colspan="2" class="overOrderbtn">去评价</td>
