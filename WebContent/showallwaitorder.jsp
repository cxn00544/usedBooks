     ...
     <tbody>
        <c:forEach var="ordermessage" items="${OrderMessageList}">
            <tr>
              ...
              <c:set var="status" value="${ordermessage.status}"/>
	   
	          <c:if test="${status == 0}">
	          	<td scope="row">等待收货</td>
	          	
	          </c:if>
                
                 <td>
	              <!--  	<button onclick="overOrder" class="overOrderbtn">退货</button> -->
	              	<a href="doAddsellbookToIndex?bookId=${ordermessage.bookId}&&userId=${user.userId}&&orderNum=${ordermessage.orderNum}" type="button" class="cancelbutton"  onclick="cancelbutton">退货</a>
	              </td>
